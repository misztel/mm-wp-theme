...
function _themename_submenu_button($dir = 'down', $title){
  ...
  $button .= '<span class="sr-only mennu-button-show">'.sprintf(esc_html__('Show %s submenu', '_themename'), $title) .'</span>';
  $button .= '<span aria-hidden="true" class="sr-only mennu-button-hide">'.sprintf(esc_html__('Hide %s submenu', '_themename'), $title) .'</span>';
